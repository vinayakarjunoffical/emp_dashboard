...
import { 
  ArrowLeft, Plus, MoreVertical, FileText, ChevronRight, X, Search, Filter, Loader2 
} from 'lucide-react';
...
const LeaveTemplate = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const mappedData = data.map(item => ({
            id: item.id,
            title: item.name,
            // ✅ Summing days_per_year from leave_types array
            totalLeaves: item.leave_types?.reduce((acc, curr) => acc + curr.days_per_year, 0) || 0,
            staffCount: 0, // Placeholder as API doesn't provide count yet
        ...
        console.error("Leave Policy API Error:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const PolicySkeleton = () => (
    <div className="bg-white border border-slate-100 rounded-xl p-6 animate-pulse shadow-sm mb-3">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="h-10 w-32 bg-slate-50 rounded-lg border border-slate-100" />
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-white font-['Inter'] pb-10 relative overflow-x-hidden text-left">
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="p-2 hover:!bg-slate-50 !bg-transparent rounded-xl !text-slate-500 transition-all border-0 cursor-pointer">
            <ArrowLeft size={18} />
          </button>
          <span className="text-[10px] font-black !text-slate-900 !capitalize tracking-widest leading-none">Settings / Leave Templates</span>
        </div>
      </div>
      ...
      <div className=" mx-auto px-2 md:px-6 mt-4">
        ...
        <div className="flex flex-col px-2 md:flex-row md:items-center justify-between gap-4 mb-4">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 px-6 py-2.5 !bg-white !text-blue-600 border border-blue-600 rounded-lg shadow-sm shadow-blue-200 hover:!bg-white transition-all active:scale-95 cursor-pointer outline-none">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-0">
                  ...
                  <div className="flex items-center gap-4 sm:gap-5">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] font-bold text-slate-500 capitalize tracking-widest">
                        Total Leaves: <span className="text-slate-900 font-black">{item.totalLeaves} Days</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4 sm:gap-8 border-t border-slate-100 sm:border-t-0 pt-3 sm:pt-0">
                    ...
                    <button className="p-2 !bg-transparent !text-slate-300 hover:!text-slate-900 transition-colors shrink-0 border-0 cursor-pointer outline-none">
