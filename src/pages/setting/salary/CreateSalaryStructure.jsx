...
const SalaryStructureTemplate = () => {
 ...
 const handleSaveTemplate = async () => {
    ...
    // ✅ Identify if we are in Fixed or Percentage mode
    const isFixedMode = calcBy.includes("Fixed Amount");

    const components = [
      ...earnings.map((item) => ({
        name: item.label,
        component_type: "earning",
        // ✅ If Percentage mode is active, parse the amount; otherwise send null
        percentage_of_ctc: !isFixedMode ? (parseFloat(item.amount) || 0) : null,
        // ✅ If Fixed mode is active, parse the amount; otherwise send null
        fixed_annual_value: isFixedMode ? (parseFloat(item.amount) || 0) : null,
      })),
      ...deductions.map((item) => ({
        name: item.label,
        component_type: "deduction",
        percentage_of_ctc: 0, // Deductions usually calculated via separate logic (PF/ESI)
    ...
    const payload = {
      name: templateName,
      description: `${staffType} template created via UI`,
      staff_type: staffType === "Monthly Regular" ? "Permanent" : staffType, // Mapping to your example "Permanent"
      calculation_type: isFixedMode ? "Fixed" : "CTC-Based", // Mapping to your example "CTC-Based"
      is_default: isDefault,
      components: components,
    };
          ...
          <div className="flex flex-col md:flex-row md:items-center mb-4 gap-8">
            <div className="space-y-2 flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-xl px-5 py-3.5 text-[11px] font-bold text-black outline-none focus:border-blue-400"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-5 py-3.5 text-[11px] font-bold text-black appearance-none outline-none focus:border-blue-400 cursor-pointer"
                >
