...
const EditSalaryStruture = () => {
  ...
  // 🔥 MATHEMATICS & TEST CASE SECURE LOGIC
      ...
      <div className="mx-auto px-6 mt-8 space-y-6 max-w-[1440px]">
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
          ...
          <div className="p-8 space-y-8">
            {deductions.map((item) => (
              // 🔥 DYNAMIC Z-INDEX FIX: If this dropdown is active, raise its z-index above subsequent rows
                      ...
                      {ptSlabs.map((slab, index) => (
                        ...
                        <div key={slab.id} className={`flex flex-col md:flex-row md:items-center justify-end gap-3 sm:gap-4 bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-xl sm:rounded-none border border-slate-100 sm:border-none relative ${activePtMenu === slab.id ? 'z-[60]' : 'z-10'}`}>
                          ...
                          <div className="flex items-center gap-2 sm:gap-3 mt-2 sm:mt-0 sm:ml-4 border-t border-slate-100 sm:border-0 pt-3 sm:pt-0 w-full sm:w-auto relative">
                            ...
                            <button onClick={() => setActivePtMenu(activePtMenu === slab.id ? null : slab.id)} className="p-1.5 !text-slate-400 hover:!text-slate-900 !bg-white sm:!bg-transparent border border-slate-200 sm:border-0 rounded-lg shrink-0 transition-colors cursor-pointer">
                            ...
                            {activePtMenu === slab.id && (
                              <>
                                ...
                                <div className="absolute right-0 top-full mt-1 w-32 bg-white border border-slate-200 rounded-xl shadow-xl z-[80] p-1.5 animate-in fade-in slide-in-from-top-1 duration-200">
                                  <button onClick={() => addPtRow(index)} className="w-full text-left px-3 py-2 text-[10px] font-black text-slate-600 hover:bg-slate-50 hover:text-blue-600 rounded-lg transition-colors uppercase tracking-widest cursor-pointer !bg-transparent border-0 outline-none">
                                    Add Row
                                  </button>
                                  <button onClick={() => deletePtRow(slab.id)} disabled={ptSlabs.length === 1} className="w-full text-left px-3 py-2 text-[10px] font-black text-rose-500 hover:bg-rose-50 hover:text-rose-600 rounded-lg transition-colors uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed mt-0.5 !bg-transparent border-0 outline-none">
                                    Delete Row
                                  </button>
                                </div>
                              </>
        ...
        <div key={modal.title} className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-in fade-in zoom-in duration-300 text-left">
            ...
            <div className="p-6 border-t border-slate-50 flex gap-4 bg-slate-50/30">
              <button onClick={() => modal.setOpen(false)} className="flex-1 py-3 !bg-white border !border-blue-600 !text-blue-600 rounded-xl text-[11px] font-black uppercase tracking-widest outline-none cursor-pointer hover:bg-slate-50">Cancel</button>
              <button onClick={modal.onSave} className="flex-1 py-3 !bg-blue-600 !text-white border border-blue-600 rounded-xl text-[11px] font-black uppercase tracking-widest shadow-sm outline-none cursor-pointer hover:bg-blue-700">Save</button>
            </div>
          </div>
        </div>
      ))}
...
//   const handleInputChange = (id, value, list, setList) => {
//     // Regex allows only numbers and decimals to prevent NaN crashes
//     const sanitizedValue = value.replace(/[^0-9.]/g, ''); 
//     setList(list.map(item => item.id === id ? { ...item, amount: sanitizedValue } : item));
//   };
...
//           <div className="bg-slate-50/50 px-8 py-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
//             <div className="flex items-center gap-3">
//               <div className="p-2 bg-blue-50 text-blue-600 rounded-lg"><Coins size={18} strokeWidth={2.5} /></div>
//               <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-[0.2em]">Earnings</h3>
//             </div>
...
//                             value={item.amount}
//                             onChange={(e) => handleInputChange(item.id, e.target.value, deductions, setDeductions)} 
//                             className="w-full pl-7 pr-3 py-2.5 bg-white border border-slate-200 rounded-lg text-[11px] font-bold text-slate-700 outline-none focus:border-blue-400 shadow-sm" 
//                           />
//                         </div>
...
//             <span className="text-[11px] font-bold text-slate-600">Gross Pay <span className="text-[9px] text-slate-400 ml-1">(Total Earnings)</span></span>
//             <span className="text-sm font-black text-slate-900">₹ {formatCurrency(grossPay)} <span className="text-[9px] text-slate-400">/ Month</span></span>
//           </div>
//           <div className="px-8 py-4 border-t border-slate-100 flex justify-between items-center bg-slate-50/80">
//             <span className="text-[11px] font-black text-slate-900">Net Pay <span className="text-[9px] font-bold text-slate-400 ml-1">(In Hand Before Taxes)</span></span>
//             <span className="text-sm font-black text-emerald-600">₹ {formatCurrency(netPay)} <span className="text-[9px] text-slate-400">/ Month</span></span>
//           </div>
//         </div> */}
...
//                             className="w-4 h-4 mr-2 rounded border-slate-300 text-blue-600 accent-blue-600" 
//                           />
//                           <span className={`text-[10px] font-black uppercase tracking-widest ${selectedEsiItems.includes(opt) ? 'text-blue-600' : 'text-slate-500'}`}>
//                             {opt}
//                           </span>
