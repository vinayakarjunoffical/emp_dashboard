...
import { useLocation, useNavigate } from "react-router-dom" // Ensure this is imported
...
const CandidateFlow = () => {
  ...
  const [isVacanciesExpanded, setIsVacanciesExpanded] = useState(false);
...
const location = useLocation();
  ...
  const loadingToast = toast.loading("Sending Decision Data...");
  ...
  try {
    ...
    // Optional: Include interview metadata if status is interview_call
    if (selectedStatus === "interview_call") {
    ...
    if (!res.ok) throw new Error("Protocol Sync Failed");

    const updatedCandidate = await res.json();
    
    // Update local state to reflect changes
    setCandidate(updatedCandidate);
    setRemark(""); // Clear remark for next entry
    
    toast.success("Decision executed successfully", { id: loadingToast });
  } catch (err) {
    console.error("PATCH Error:", err);
    toast.error("Access Denied: Update Failed", { id: loadingToast });
  }
};
...
<div className="flex items-center justify-between mb-8">
  <button 
    onClick={() => navigate(-1)} 
    className="group flex items-center gap-2.5 px-5 py-2.5 !bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] !text-slate-500 hover:!text-blue-600 hover:!border-blue-200 transition-all shadow-sm active:scale-95"
  >
  ...
  <div className="hidden md:flex items-center gap-3">
     <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 px-3 py-1 rounded-lg">
       Module: Decision
     </span>
  </div>
</div>
...
<div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 relative overflow-hidden group">
  ...
  <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
   ...
   <div className="bg-slate-50 px-8 py-4 rounded-2xl border border-slate-100 min-w-[280px]">
  <span className={labelClass}>Applied Vacancies</span>
  
  <div className="flex flex-wrap items-center gap-1.5 mt-2">
    <Briefcase size={14} className="text-blue-600 mr-1 shrink-0" />
    
    <div className="text-[11px] font-black text-slate-700 uppercase tracking-tight">
      {candidate.applied_vacancies && candidate.applied_vacancies.length > 0 ? (
        <>
          {/* Logic: Show first 2 if collapsed, show all if expanded */}
          {(isVacanciesExpanded 
            ? candidate.applied_vacancies 
            : candidate.applied_vacancies.slice(0, 2)
          ).map((v, idx) => (
            <span key={v.id}>
              {v.title}
              {idx < (isVacanciesExpanded ? candidate.applied_vacancies.length : 2) - 1 && 
               idx < candidate.applied_vacancies.length - 1 ? ", " : ""}
            </span>
          ))}

          {/* Toggle Button for More/Less */}
          {candidate.applied_vacancies.length > 2 && (
            <button
              onClick={() => setIsVacanciesExpanded(!isVacanciesExpanded)}
              className="ml-2 px-2 py-0.5 bg-blue-50 text-blue-600 rounded-md text-[9px] font-black border border-blue-100 hover:bg-blue-600 hover:text-white transition-all cursor-pointer inline-flex items-center gap-1"
            >
              {isVacanciesExpanded ? (
                "Show Less"
              ) : (
                `+${candidate.applied_vacancies.length - 2} More`
              )}
            </button>
          )}
        </>
      ) : (
        <span className="text-slate-400 italic">No Vacancies Linked</span>
      )}
    </div>
  </div>
</div>
            ...
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-2">
                  <label className={labelClass}>Select Decision Type</label>
                ...
                className="w-full bg-[#2563eb] hover:bg-blue-700 text-white py-5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] transition-all shadow-xl shadow-blue-100 active:scale-[0.99] flex items-center justify-center gap-3">
          ...
          <div className="lg:col-span-5 bg-white rounded-[2rem] border border-slate-100 shadow-sm flex flex-col h-fit">
            <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.15em]">Candidate Remarks</h3>
              </div>
              <span className="text-[9px] font-black text-slate-400 bg-slate-50 px-2 py-1 rounded border border-slate-100">{candidate.length} Logs</span>
            </div>
            ...
            {/* Replace mock previousDecisions with candidate history if available */}
<div className="p-8 space-y-8 overflow-y-auto max-h-[650px] custom-scrollbar">
  {candidate.history?.length > 0 ? (
    candidate.history.map((item, idx) => (
      <div key={idx} className="relative pl-8 border-l-2 border-slate-100 last:border-l-0 pb-2">
        {/* ... Card Content ... */}
        <span className="text-[12px] font-black text-slate-900 uppercase">{item.status}</span>
        <p className="text-[11px] text-slate-500 italic">"{item.remark}"</p>
      </div>
    ))
  ) : (
