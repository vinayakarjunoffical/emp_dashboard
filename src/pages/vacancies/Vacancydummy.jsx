...
const [vacancyMetrics, setVacancyMetrics] = useState({});
const [jobTemplates, setJobTemplates] = useState([]);
const [filteredTemplates, setFilteredTemplates] = useState([]);
const [showSuggestions, setShowSuggestions] = useState(false);
const [noMatchFound, setNoMatchFound] = useState(false);
const [isStatusModalOpen, setIsStatusModalOpen] = useState(false);
const [selectedVacancyForStatus, setSelectedVacancyForStatus] = useState(null);
const [newStatus, setNewStatus] = useState("");
const [showAddForm, setShowAddForm] = useState(false); // Default hidden
const [filterStatus, setFilterStatus] = useState("all"); // options: 'all', 'open', 'closed', 'on_hold'
...
useEffect(() => {
  const fetchTemplates = async () => {
    try {
      const res = await fetch("https://apihrr.goelectronix.co.in/job-descriptions");
      const data = await res.json();
      setJobTemplates(data || []);
    } catch (err) {
      console.error("Template Registry Sync Failed");
    }
  };
  fetchTemplates();
}, []);
...
useEffect(() => {
  ...
  const typingTimer = setTimeout(async () => {
    try {
      ...
      setJobTemplates(allTemplates || []);

      const searchTerm = formData.title.trim().toLowerCase();
      const exactMatch = allTemplates.find(t => t.title.trim().toLowerCase() === searchTerm);

      if (exactMatch) {
        setFormData(prev => ({
          ...prev,
          content: exactMatch.content,
          responsibilities: exactMatch.responsibilities,
          requirements: exactMatch.requirements,
          job_description_id: exactMatch.id
        }));
        setNoMatchFound(false);
      } else {
        // ✅ No match? Signal the UI to show the extra selector
...
// Add this helper to your component if not already there
const filteredTemplateSuggestions = useMemo(() => {
  if (!formData.title.trim()) return [];
  return jobTemplates.filter(t => 
    t.title.toLowerCase().includes(formData.title.toLowerCase())
  );
}, [formData.title, jobTemplates]);
...
const handleSubmit = async (e) => {
  ...
  // --- NEW: AGE VALIDATION PROTOCOL ---
  if (activeDetails.includes('age')) {
    if (formData.min_age < 18) {
      toast.error("Minimum age protocol requires 18+");
      return;
    }
    if (formData.max_age > 60) {
      toast.error("Maximum age exceeds employment registry limits (60)");
      return;
    }
    if (formData.min_age > formData.max_age) {
      toast.error("Invalid Range: Min age cannot exceed Max age");
      return;
    }
  }
...
city: Array.isArray(formData.city) ? formData.city : [formData.city],
  ...
  cand_type: formData.cand_type || "Fresher",
...
const toggleAccordion = async (vacancy) => {
  const isOpening = openAccordionId !== vacancy.id;
  setOpenAccordionId(isOpening ? vacancy.id : null);

  if (isOpening) {
    // 1. Fetch JD Details (Existing)
    if (vacancy.job_description_id) {
      fetchJDDetails(vacancy.job_description_id);
    }

    // 2. Fetch Metrics (New Logic)
    try {
      const [resResponses, resLeads, resDatabase] = await Promise.all([
        // Responses: candidates with jd_sent status
        fetch(`https://apihrr.goelectronix.co.in/candidates?status=jd_sent&vacancy_id=${vacancy.id}`),
        // Hot Leads: matching vacancy_id
        fetch(`https://apihrr.goelectronix.co.in/candidates?vacancy_id=${vacancy.id}`),
      ...
      const dataDatabase = await resDatabase.json();

      setVacancyMetrics(prev => ({
        ...prev,
        [vacancy.id]: {
          responses: dataResponses.length || 0,
          leads: dataLeads.length || 0,
          database: dataDatabase.length || 0
        }
      }));
    } catch (err) {
...
const handleStatusTransition = async () => {
  if (!selectedVacancyForStatus || !newStatus) return;

  const loadingToast = toast.loading("Updating Status...");
  try {
    // We send the existing vacancy data + the new status
    const response = await fetch(`https://apihrr.goelectronix.co.in/vacancies/${selectedVacancyForStatus.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...selectedVacancyForStatus, // Spread existing data to satisfy API requirements
        status: newStatus.toLowerCase() // 'open', 'closed', or 'on_hold'
      }),
    });

    if (!response.ok) throw new Error("Transition Failed");

    toast.success(`Vacancy is now ${newStatus.toUpperCase()}`, { id: loadingToast });
    setIsStatusModalOpen(false);
    fetchVacancies(); // Refresh list to show updated status
  } catch (err) {
    toast.error("Status  failed", { id: loadingToast });
  }
};
...
const filteredVacancies = useMemo(() => {
  if (filterStatus === "all") return vacancies;
  return vacancies.filter(v => v.status === filterStatus);
}, [vacancies, filterStatus]);
...
const StatusMetricCard = ({ icon: Icon, label, count, subLabel, colorClass, iconBg, onClick }) => (
  <div 
    onClick={onClick}
    className="flex-1 bg-white border border-slate-200 rounded-2xl p-5 flex items-center justify-between group hover:border-blue-300 hover:shadow-md transition-all cursor-pointer active:scale-[0.98]"
  >
    <div className="flex items-center gap-4">
      <div className={`h-12 w-12 ${iconBg} rounded-xl flex items-center justify-center transition-transform group-hover:scale-110`}>
        <Icon className={colorClass} size={22} />
      </div>
      <div>
        <div className="flex items-center gap-2">
          <span className="text-lg font-black text-slate-900 leading-none">{count}</span>
          <span className="text-[11px] font-bold text-slate-800 tracking-tight">{label}</span>
        </div>
...
<div className="bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
  ...
  <div className="flex items-center gap-6 relative z-10">
    ...
    <div>
      ...
      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
        <Layers size={12} className="text-blue-500" /> Add Vacancy Details
      </p>
    </div>
  </div>
  ...
  className={`relative z-10 flex items-center gap-3 px-8 py-3.5 rounded-[1.25rem] text-[10px] font-black uppercase tracking-[0.25em] transition-all duration-300 active:scale-95 shadow-sm border-2 ${
    showAddForm 
    ? "!bg-white !border-blue-600 !text-blue-500 hover:!bg-slate-50 !shadow-slate-200" 
    : "!bg-white !border-[#2563eb] !text-[#2563eb] hover:!bg-blue-50 hover:shadow-blue-100/50 shadow-blue-50"
  }`}
>
  {showAddForm ? (
    <> 
      <X size={16} strokeWidth={3} className="animate-in fade-in zoom-in duration-300" /> 
      Close Node 
    </>
  ) : (
    <> 
      {/* Branding Box inside button for that professional SaaS feel */}
      <div className="p-1 bg-blue-50 rounded-lg group-hover:bg-white transition-colors">
...
<div className="space-y-5">
  {/* EXISTING JOB TITLE INPUT WITH SUGGESTIONS */}
  <div className="relative">
    <label className={labelClass}>Job Title</label>
    <div className="relative group">
      <input
        required
        placeholder="Job Title (e.g. Node JS)"
        className={`${inputClass} ${!noMatchFound && formData.title && filteredTemplateSuggestions.length > 0 ? 'rounded-b-none border-blue-400' : ''}`}
        value={formData.title}
        onChange={(e) => setFormData({ ...formData, title: e.target.value })}
      />
      
      {/* 🟢 NEW: SUGGESTION LIST */}
{!noMatchFound && formData.title.length > 0 && filteredTemplateSuggestions.length > 0 && (
  /* We add a check to hide if the input exactly matches a selected template */
  formData.title !== filteredTemplateSuggestions[0]?.title && (
    <div className="absolute z-[100] w-full bg-white border-x border-b border-slate-200 rounded-b-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-1">
      <div className="p-2 bg-slate-50 border-b border-slate-100">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-2">Quick Select Template</p>
      </div>
      <div className="max-h-48 overflow-y-auto custom-scrollbar">
        {filteredTemplateSuggestions.map((t) => (
          <button
            key={t.id}
            type="button"
            onClick={() => {
              setFormData({
                ...
                job_description_id: t.id
              });
              
              // This is the "Close" trigger
              setNoMatchFound(false); 
          ...
          >
            ...
            <Check size={12} className="text-emerald-500 opacity-0 group-hover/item:opacity-100 transition-opacity" />
          </button>
        ))}
      </div>
    </div>
  ...
  {/* ✅ EXTRA INPUT: (Your existing code stays exactly here) */}
  {noMatchFound && (
    <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl animate-in slide-in-from-top-2 duration-300">
      ...
      <div className="relative">
        <select
          className={`${inputClass} !bg-white border-blue-200 pr-10 appearance-none`}
          value={formData.job_description_id || ""}
          onChange={(e) => {
            const selected = jobTemplates.find(t => t.id.toString() === e.target.value);
            if (selected) {
              setFormData({
                ...formData,
                ...
                job_description_id: selected.id
              });
              toast.success("Content Protocol Linked");
              setNoMatchFound(false); // Hide the blue box after successful selection
            }
          }}
        >
        ...
        <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-blue-500 pointer-events-none" />
              ...
              {/* GEOLOCATION ENGINE - DEPENDENT DROPDOWNS */}
        ...
        <select 
          ...
          onChange={(e) => {
            ...
            setFormData({
              ...
              location: [selectedDist] // Sync location with district
            });
          }}
        >
          <option value="">Select District</option>
          {Object.keys(DISTRICT_CITY_MAP).map(dist => (
            <option key={dist} value={dist}>{dist}</option>
          ))}
        </select>
        <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none group-hover:text-blue-600 transition-colors" />
      </div>
    </div>

    {/* CITY SELECTOR (Dependent on District) */}
    <div>
      <label className={labelClass}>City</label>
      <div className="relative group">
        <select 
          disabled={!formData.district[0]}
          className={`${inputClass} appearance-none pr-10 ${!formData.district[0] ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          onChange={(e) => {
            ...
            setFormData({
              ...formData,
              city: [selectedCity],
              location: [`${selectedCity}, ${formData.district[0]}`] // Full location protocol
            });
          }}
        >
          <option value="">{formData.district[0] ? "Select City" : "Select District First"}</option>
          {formData.district[0] && DISTRICT_CITY_MAP[formData.district[0]].map(city => (
...
<div className="space-y-6 pt-6 border-t border-slate-100">
  ...
  <div className="space-y-6">
    ...
    {/* AGE SECTION - ENFORCING ENTERPRISE CONSTRAINTS (18-60) */}
{activeDetails.includes('age') && (
  <div className="p-6 bg-white border border-slate-100 rounded-[2rem] shadow-sm animate-in slide-in-from-top-2 duration-300 relative group">
    ...
    <button 
      ...
      onClick={() => {
        setActiveDetails(prev => prev.filter(i => i !== 'age'));
        setFormData({ ...formData, min_age: 0, max_age: 0 }); // Reset data on remove
      }}
    >
    ...
    <div className="flex items-center gap-2">
      <label className={labelClass}>Candidate Age Eligibility</label>
      <span className="text-[8px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 uppercase mb-2">
        Protocol: 18 - 60 Years
      </span>
    </div>
    ...
    <div className="grid grid-cols-2 gap-8 mt-2">
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <input 
            type="number" 
            placeholder="Min (18+)" 
            className={`${inputClass} ${formData.min_age && formData.min_age < 18 ? 'border-red-300 bg-red-50/30' : ''}`}
            value={formData.min_age || ""}
            onChange={(e) => {
              const val = parseInt(e.target.value);
              // Enforcement: Don't allow value above 60
              if (val > 60) return; 
              setFormData({ ...formData, min_age: val });
            }}
          />
          {formData.min_age > 0 && formData.min_age < 18 && (
            <p className="text-[8px] font-black text-red-500 uppercase mt-1.5 ml-1 animate-pulse">
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <input 
            type="number" 
            placeholder="Max (Up to 60)" 
            className={`${inputClass} ${formData.max_age > 60 ? 'border-red-300 bg-red-50/30' : ''}`}
            value={formData.max_age || ""}
            onChange={(e) => {
              const val = parseInt(e.target.value);
              // Enforcement: Hard cap at 99 or similar to prevent UI break, 
              // but validation happens on the limit
              setFormData({ ...formData, max_age: val });
            }}
          />
          {formData.max_age > 60 && (
            <p className="text-[8px] font-black text-red-500 uppercase mt-1.5 ml-1 animate-pulse">
        ...
        return (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-tight border transition-all duration-200 ${
              isSelected
                ? "!bg-white !border-[#2563eb] !text-blue-600 "
                : "!bg-white !border-slate-200 !text-slate-500 hover:border-slate-400"
            }`}
          >
            {lang}
      ...
      <div className="flex flex-wrap gap-2 min-h-[40px]">
        {formData.degree_ids.map((eduId) => {
          ...
          return (
            <span 
              key={eduId}
              className="flex items-center gap-2 px-3 py-1.5 !bg-white text-blue-600 border border-blue-600 rounded-xl text-[10px] font-black uppercase tracking-wider animate-in zoom-in-95"
            >
              {edu?.name}
              <X 
                size={12} 
    ...
    <div className="space-y-4">
      ...
      <div className="min-h-[40px]">
        {formData.industry_id ? (
          <div className="flex items-center gap-2 w-fit px-4 py-2 !bg-white text-blue-600 border border-blue-600 rounded-xl text-[11px] font-black uppercase tracking-wider animate-in zoom-in-95">
            <Layers size={14} />
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2.5 min-h-[40px]">
        {formData.certificates_req.map((certName, index) => (
          <div 
            key={index}
            className="flex items-center gap-2 px-3.5 py-2 !bg-white text-blue-600 border border-blue-600 rounded-xl text-[10px] font-black uppercase tracking-wider animate-in zoom-in-95 shadow-md shadow-blue-50"
          >
            {certName}
            <X 
              size={13} 
...
>
  ...
  {/* ✅ 2. DYNAMIC STATUS PROTOCOL (Right Aligned) */}
  <div className={`hidden lg:flex items-center gap-2.5 px-4 py-2 rounded-2xl border transition-all duration-500 shadow-sm ${
    vacancy?.status === 'open' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 
    vacancy?.status === 'closed' ? 'bg-red-50 border-red-100 text-red-600' : 
    'bg-orange-50 border-orange-100 text-orange-500'
  }`}>
    <div className="flex flex-col items-end">
      {/* <span className="text-[8px] font-black uppercase tracking-[0.2em] opacity-60 leading-none mb-1">Status</span> */}
      <div className="flex items-center gap-2">
  ...
  {/* 3. ACCORDION CONTROL NODE */}
  <div className={`p-2 rounded-xl transition-all duration-500 ${
    openAccordionId === vacancy.id ? "bg-blue-600 text-white shadow-lg rotate-180" : "text-slate-300 group-hover:text-blue-600 bg-slate-50"
  }`}>
...
{openAccordionId === vacancy.id && (
  <div className="px-8 pb-10 pt-6 border-t border-slate-50 bg-slate-50/30 animate-in slide-in-from-top-2 duration-500">
    <div className="space-y-10">
      
      {/* --- NEW: STATUS METRIC GRID (3 TABS/CARDS) --- */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <StatusMetricCard 
    icon={Users} 
    // Use dynamic count, fallback to "0" while loading
    count={vacancyMetrics[vacancy.id]?.responses ?? "..."} 
    label="Responses" 
    subLabel="JD Sent Protocol"
    iconBg="bg-blue-50"
    colorClass="text-blue-600"
    onClick={() => navigate(`/candidatefilter?status=jd_sent&vacancy_id=${vacancy.id}`)}
  />
  <StatusMetricCard 
    icon={Zap} 
    count={vacancyMetrics[vacancy.id]?.leads ?? "..."} 
    label="Hot Leads" 
    subLabel="Match your job post"
    iconBg="bg-orange-50"
    colorClass="text-orange-500"
    onClick={() => navigate(`/candidatefilter?vacancy_id=${vacancy.id}`)}
  />
  <StatusMetricCard 
    icon={ShieldCheck} 
    count={vacancyMetrics[vacancy.id]?.database ?? "..."} 
    label="Candidates" 
    subLabel="Global Registry"
    iconBg="bg-slate-50"
  ...
  />
      ...
      {/* EXISTING TECHNICAL DETAILS GRID */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-4">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <div>
              <p className="text-[9px] font-bold text-slate-400 uppercase">Age Range</p>
              <p className="text-xs font-bold text-slate-700">{vacancy.min_age} - {vacancy.max_age} Years</p>
            </div>
            <div>
              <p className="text-[9px] font-bold text-slate-400 uppercase">Assets</p>
              <div className="flex flex-wrap gap-1.5 mt-1">
                {vacancy.assets_req?.map(asset => (
                  <span key={asset} className="px-2 py-0.5 bg-slate-50 border border-slate-200 rounded text-[9px] font-bold text-slate-500 uppercase">{asset}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
            ...
            <div className="pt-2 border-t border-slate-50">
              <p className="text-[9px] font-bold text-slate-400 uppercase">Language Known</p>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
            ...
            <div className="pt-1">
              <p className="text-[9px] font-bold text-slate-400 uppercase">Calling Time</p>
              <p className="text-[10px] font-black text-slate-700 uppercase leading-relaxed">
                {vacancy.cand_can_call} <br/> 
                <span className="text-blue-600">{vacancy.call_timings}</span>
              </p>
            </div>
          </div>
        </div>
      ...
      {/* FULL JD LINK */}
      <div className="mt-4 flex justify-between pt-6 border-t border-slate-200/60">
        <button 
          onClick={() => navigate(`/vacancy-details/${vacancy.id}`)} 
          className="group flex !bg-transparent items-center gap-3 text-[11px] font-black !text-[#2563eb] uppercase tracking-widest hover:!text-blue-800 transition-all"
        >
           Read full job description <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
        </button>

       <div className="flex gap-4">
         {/* <button 
    onClick={() => navigate(`/edit-vacancy/${vacancy.id}`)}
    className="px-6 py-3 bg-white border border-slate-200 text-slate-500 rounded-xl text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-2 hover:bg-slate-50 transition-all active:scale-95"
  >
    <Edit3 size={14} strokeWidth={3} /> Edit Vacancy
  </button> */}

  <button 
  onClick={() => {
    setSelectedVacancyForStatus(vacancy);
    setNewStatus(vacancy.status);
    setIsStatusModalOpen(true);
  }}
  className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-2 hover:bg-black transition-all active:scale-95 shadow-lg"
>
  <Activity size={14} strokeWidth={3} /> Change Status
</button>
       </div>
      </div>
    </div>
...
{isStatusModalOpen && (
  <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 animate-in fade-in duration-300">
    <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" onClick={() => setIsStatusModalOpen(false)} />
    
    <div className="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
      <div className="bg-slate-50 px-8 py-6 border-b border-slate-100 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase">Vacancy Status Change</p>
        </div>
      </div>

      <div className="p-8 space-y-6">
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Current Status: {selectedVacancyForStatus?.status}</label>
          <div className="relative group">
            <select 
              value={newStatus}
              onChange={(e) => setNewStatus(e.target.value)}
              className="w-full pl-4 pr-10 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-black text-slate-700 uppercase tracking-widest outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-600 transition-all appearance-none cursor-pointer"
            >
      ...
      <div className="p-6 bg-slate-100 flex gap-3">
        <button onClick={() => setIsStatusModalOpen(false)} className="flex-1 px-6 py-3 bg-slate-800 text-slate-400 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-700 transition-all">
          Close
        </button>
        <button onClick={handleStatusTransition} className="flex-[2] bg-blue-600 text-white py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 transition-all flex items-center justify-center gap-2">
          Change Status <Check size={14} strokeWidth={4} />
        </button>
      </div>
    </div>
  </div>
...
//       // district: Array.isArray(formData.district) ? formData.district : [formData.district],
//       // city: Array.isArray(formData.city) ? formData.city : [formData.city],
//       // From your existing handleSubmit Stage 2:
// district: Array.isArray(formData.district) ? formData.district : [formData.district],
// city: Array.isArray(formData.city) ? formData.city : [formData.city],
//       pincode: Array.isArray(formData.pincode) ? formData.pincode : [formData.pincode],
//       job_type: formData.job_type,
...
//   {/* ✅ ADD VACANCY TOGGLE BUTTON */}
//   {/* <button
//     onClick={() => setShowAddForm(!showAddForm)}
//     className={`relative z-10 flex items-center gap-3 px-8 py-4 rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 shadow-lg ${
//       showAddForm 
...
//   >
//     {showAddForm ? (
//       <> <X size={18} strokeWidth={3} /> Close Form </>
//     ) : (
//       <> <PlusCircle size={18} strokeWidth={2.5} /> Add Vacancy </>
//     )}
//   </button> */}
//   <button
//   onClick={() => setShowAddForm(!showAddForm)}
...
//       required
//       placeholder="Job Title"
//       // Added dynamic border radius when dropdown is open
//       className={`${inputClass} ${showSuggestions ? 'rounded-b-none border-blue-400' : ''}`}
//       value={formData.title}
//       onChange={(e) =>
//         setFormData({ ...formData, title: e.target.value })
//       }
...
//        <div className="h-1 w-1 rounded-full bg-blue-600 animate-ping opacity-0 group-focus-within:opacity-100" />
//     </div>
//   </div>
...
//     <div className="absolute z-[100] w-full bg-white border-x border-b border-slate-200 rounded-b-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-1">
//       <div className="p-2 bg-slate-50 border-b border-slate-100">
//         <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-2">Matching Templates</p>
//       </div>
//       <div className="max-h-48 overflow-y-auto custom-scrollbar">
...
//               toast.success("Template Protocol Applied");
//             }}
//             className="w-full flex items-center justify-between px-4 py-3 hover:bg-blue-50 transition-colors text-left group/item border-b border-slate-50 last:border-0"
//           >
//             <div className="flex items-center gap-3">
//               <div className="p-1.5 bg-slate-100 rounded-lg group-hover/item:bg-white text-slate-400 group-hover/item:text-blue-600 transition-all">
//                 <FileText size={12} />
//               </div>
//               <span className="text-[11px] font-bold text-slate-700 uppercase">{template.title}</span>
//             </div>
//             <ArrowRight size={12} className="text-slate-300 group-hover/item:text-blue-600 group-hover/item:translate-x-1 transition-all" />
//           </button>
//         ))}
//       </div>
//     </div>
...
//       {/* 🟢 NEW: SUGGESTION LIST - Appears while typing */}
//       {/* {!noMatchFound && formData.title.length > 0 && filteredTemplateSuggestions.length > 0 && (
//         <div className="absolute z-[100] w-full bg-white border-x border-b border-slate-200 rounded-b-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-1">
//           <div className="p-2 bg-slate-50 border-b border-slate-100">
//             <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-2">Quick Select Template</p>
...
//                   setFormData({
//                     ...formData,
//                     title: t.title, // Sync title
//                     content: t.content,
//                     responsibilities: t.responsibilities,
//                     requirements: t.requirements,
//                     job_description_id: t.id
//                   });
//                   setNoMatchFound(false); // Hide the extra select box since we matched
//                   toast.success(`Protocol ${t.title} Linked`);
//                 }}
//                 className="w-full flex items-center justify-between px-4 py-3 hover:bg-blue-50 transition-colors text-left group/item border-b border-slate-50 last:border-0"
//               >
...
//               // Logic to clear the filtered list immediately
//               // if your filteredTemplateSuggestions depends on an 'isDirty' flag
...
//       navigate(`/edit-vacancy/${vacancy.id}`); // 🚀 Redirect to edit page
//     }}
//     className="group/edit p-3 bg-white border border-slate-200 text-slate-400 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 rounded-xl transition-all shadow-sm active:scale-95 flex items-center gap-2"
//     title="Edit Vacancy Node"
//   >
//     <Edit3 size={18} strokeWidth={2.5} />
//     <span className="text-[10px] font-black uppercase tracking-tighter hidden lg:block">Edit Vacancy</span>
//   </button>
...
// // useEffect(() => {
// //   // 1. If input is empty, don't trigger the sync
// //   if (!formData.title.trim()) return;

// //   // 2. Set a timer (e.g., 600ms) to wait for the user to stop typing
// //   const typingTimer = setTimeout(async () => {
// //     try {
// //       // 3. Call the Template API to check for matches
// //       // Note: We call the API here to ensure we have the absolute latest templates
// //       const res = await fetch(`https://apihrr.goelectronix.co.in/job-descriptions`);
// //       const allTemplates = await res.json();

// //       // 4. Fuzzy match protocol
// //       const match = allTemplates.find(
// //         (t) => t.title.trim().toLowerCase() === formData.title.trim().toLowerCase()
// //       );
...
// //           job_description_id: match.id
// //         }));

// //         toast.success(`Registry Linked: ${match.title.toUpperCase()}`, {
// //           icon: '📋',
// //           style: { borderRadius: '12px', fontSize: '10px', fontWeight: '900' }
// //         });
// //       }
// //     } catch (err) {
// //       console.error("Auto-Hydration Protocol Failed");
// //     }
// //   }, 600); // 600ms delay = "Typing Completed"

// //   // 5. CLEANUP: This is the most important part. 
// //   // It cancels the previous timer if the user types another letter.
// //   return () => clearTimeout(typingTimer);
...
// //         setShowSuggestions(false);
// //       } else {
// //         // 2. No exact match? Filter list for the dropdown
// //         const suggestions = allTemplates.filter(t => 
// //           t.title.toLowerCase().includes(formData.title.toLowerCase())
// //         );
// //         setFilteredTemplates(suggestions);
...
// //       // 1. Check for Exact Match
// //       const exactMatch = allTemplates.find(
// //         (t) => t.title.trim().toLowerCase() === searchTerm
// //       );

// //       if (exactMatch) {
// //         // Auto-fill and hide dropdown if perfect match found
// //         setFormData(prev => ({
// //           ...prev,
// //           content: exactMatch.content,
// //           responsibilities: exactMatch.responsibilities,
...
// //         setShowSuggestions(false);
// //       } else {
// //         // 2. No exact match? Filter for suggestions
// //         const suggestions = allTemplates.filter(t => 
// //           t.title.toLowerCase().includes(searchTerm)
// //         );
        
// //         setFilteredTemplates(suggestions);
// //         // Show dropdown only if we have suggestions and user is still focused
// //         setShowSuggestions(suggestions.length > 0);
// //       }
// //     } catch (err) {
// //       console.error("Template Sync Failed");
...
//   {/* ✅ EXTRA INPUT: Appears only if Title doesn't match a template */}
//   {noMatchFound && (
//     <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl animate-in slide-in-from-top-2 duration-300">
//       <div className="flex items-center gap-2 mb-3">
//         <Info size={14} className="text-blue-600" />
...
//     iconBg="bg-blue-50"
//     colorClass="text-blue-600"
//     onClick={() => navigate(`/candidate?status=jd_sent`)}
//   />
//   <StatusMetricCard 
//     icon={Zap} 
//     count={vacancyMetrics[vacancy.id]?.leads ?? "..."} 
...
//     iconBg="bg-orange-50"
//     colorClass="text-orange-500"
//     onClick={() => navigate(`/candidate?vacancy_id=${vacancy.id}`)}
//   />
//   <StatusMetricCard 
//     icon={ShieldCheck} 
//     count={vacancyMetrics[vacancy.id]?.database ?? "..."} 
...
//             </div>
//             <div className="pt-2 border-t border-slate-50">
//               <p className="text-[9px] font-bold text-slate-400 uppercase">Language Proficiency</p>
//               <p className="text-xs font-bold text-slate-700 uppercase">{vacancy.spoken_languages?.join(", ")}</p>
//             </div>
//           </div>
//         </div>
...
//             )}
//             <div className="pt-1">
//               <p className="text-[9px] font-bold text-slate-400 uppercase">Calling Window</p>
//               <p className="text-[10px] font-black text-slate-700 uppercase leading-relaxed">
//                 {vacancy.cand_can_call} <br/> 
//                 <span className="text-blue-600">{vacancy.call_timings}</span>
//               </p>
...
// //     const StatusMetricCard = ({ icon: Icon, label, count, subLabel, colorClass, iconBg }) => (
// //   <div className="flex-1 bg-white border border-slate-200 rounded-2xl p-5 flex items-center justify-between group hover:border-blue-300 hover:shadow-md transition-all cursor-pointer">
// //     <div className="flex items-center gap-4">
// //       <div className={`h-12 w-12 ${iconBg} rounded-xl flex items-center justify-center transition-transform group-hover:scale-110`}>
// //         <Icon className={colorClass} size={22} />
// //       </div>
