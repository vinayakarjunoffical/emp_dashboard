...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-1">
  ...
  onClick={(e) => {
    ...
    setDecisionCandidate(c); // 🎯 This opens the modal
  }}
  className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-6 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] hover:border-blue-600 hover:text-blue-600 transition-all shadow-sm active:scale-95"
>
  <Gavel size={14} /> Decision
  ...
  <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 animate-in fade-in duration-300">
    <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setDecisionCandidate(null)} />
    
    <div className="relative bg-white w-full max-w-4xl rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col h-[700px]">
      ...
      <div className="bg-slate-50 px-8 py-6 border-b border-slate-100 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">Follow Up Protocol</h3>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{decisionCandidate.full_name}</p>
          </div>
        </div>
        <button onClick={() => setDecisionCandidate(null)} className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-400">
          <X size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 overflow-hidden">
        ...
        {/* 🟢 LEFT SIDE: UPDATE FORM (Scrollable) */}
        <div className="p-8 space-y-6 overflow-y-auto custom-scrollbar border-r border-slate-100 bg-white">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Update Registry</h4>
          </div>
          ...
          {(['interview', 'not_connect', 'reschedule'].includes(decisionData.status)) && (
            <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2 duration-300">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Date</label>
                <input type="date" className="w-full px-4 py-3 bg-white border border-blue-100 rounded-xl text-xs font-bold outline-none focus:border-blue-600" value={decisionData.follow_up_date} onChange={(e) => setDecisionData({...decisionData, follow_up_date: e.target.value})} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Time</label>
                <input type="time" className="w-full px-4 py-3 bg-white border border-blue-100 rounded-xl text-xs font-bold outline-none focus:border-blue-600" value={decisionData.follow_up_time} onChange={(e) => setDecisionData({...decisionData, follow_up_time: e.target.value})} />
              </div>
            </div>
          )}

          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Interaction Remarks</label>
            <textarea rows={4} placeholder="Type current call interaction details..." className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-[13px] font-bold outline-none focus:bg-white focus:border-blue-600 transition-all resize-none shadow-inner" value={decisionData.remark} onChange={(e) => setDecisionData({...decisionData, remark: e.target.value})} />
          </div>
        </div>

        {/* 🔵 RIGHT SIDE: REGISTRY HISTORY (Independently Scrollable) */}
        <div className="bg-slate-50/50 p-8 flex flex-col h-full overflow-hidden">
          <div className="flex items-center justify-between mb-6 shrink-0">
            <div className="flex items-center gap-2">
              <div className="p-2 bg-white rounded-lg shadow-sm text-slate-400 border border-slate-100"><Clock size={16} strokeWidth={2.5} /></div>
              <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Audit Trail</h4>
            </div>
            <span className="text-[8px] font-black bg-blue-100 text-blue-600 px-2.5 py-1 rounded-md uppercase border border-blue-200 shadow-sm">4 Nodes</span>
          </div>

          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
            <div className="pl-2 space-y-10 relative border-l-2 border-slate-200 ml-2 py-4">
              {[
                { status: 'interview', label: 'Call For Interview', date: '04-Mar', time: '10:30 AM', author: 'Sujit H.', remark: 'Scheduled technical round. Candidate seems promising.', icon: <Phone size={10} />, color: 'text-blue-600', bg: 'bg-blue-50' },
                { status: 'not_connect', label: 'Not Connected', date: '02-Mar', time: '02:15 PM', author: 'System Admin', remark: 'Registry note: Number busy, scheduled retry.', icon: <X size={10} />, color: 'text-orange-500', bg: 'bg-orange-50' }
              ].map((item, idx) => (
                <div key={idx} className="relative pl-8 animate-in fade-in slide-in-from-left-3 duration-700">
                  <div className={`absolute -left-[11px] top-1 h-5 w-5 rounded-full ${item.bg} border-4 border-white shadow-md flex items-center justify-center ${item.color} z-10`}>{item.icon}</div>
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm group/card relative overflow-hidden">
                      <div className="flex items-center justify-between mb-3 pb-3 border-b border-slate-50">
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-tight">By: <span className="text-slate-900">{item.author}</span></span>
                        <span className="text-[9px] font-black text-slate-400 uppercase">{item.date} • {item.time}</span>
                      </div>
                      <p className="text-[11px] font-medium text-slate-600 italic">"{item.remark}"</p>
                      <div className={`absolute right-0 top-0 bottom-0 w-1 ${item.color.replace('text', 'bg')}`} />
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="p-6 bg-slate-100 border-t border-slate-200 flex justify-end items-center gap-3 shrink-0">
        <button onClick={() => setDecisionCandidate(null)} className="px-6 py-3 text-[10px] rounded-xl font-black uppercase text-slate-400 hover:text-slate-600 transition-all active:scale-95">Discard</button>
        <button onClick={() => navigate(`/profile/${decisionCandidate.id}`)} className="px-5 py-3 !bg-white border !border-blue-600 !text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-all flex items-center justify-center gap-2 active:scale-95 shadow-sm">
        ...
        <button 
          ...
          className="px-8 py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 shadow-lg shadow-blue-200 flex items-center justify-center gap-2 active:scale-95"
        >
