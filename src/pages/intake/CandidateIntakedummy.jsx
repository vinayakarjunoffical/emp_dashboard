...
const experienceOptions = useMemo(() => {
  // If no candidates yet, show a default range to keep the UI clean
...
//         filters.educations.includes(
//           normalizeText(c.highestDegree).toUpperCase(),
//         );

//       const matchesCity =
//         filters.cities.length === 0 ||
//         filters.cities.some((filterVal) => {
//           const candidateFullLoc =
//             `${c.city || ""}${district ? `, ${district}` : ""}, ${c.country || "INDIA"}`.toUpperCase();
//           return filterVal === candidateFullLoc;
//         });
//       let matchesExperience = true;
//       if (filters.experiences.length > 0) {
//         const candidateExp = parseFloat(c.total_experience_years || 0);

//         matchesExperience = filters.experiences.some((filterLabel) => {
//           // Extract the number from "2 YEARS" -> 2
//           const maxAllowedExp = parseInt(filterLabel.replace(/\D/g, ""));

//           // Logic: Show candidate if their experience is less than or equal to selected
//           // Example: Select 2 -> Shows 0, 1.5, 2.0
//           return candidateExp <= maxAllowedExp;
//         });
//       }
...
//         const age = parseInt(c.age || 0); // Use flat 'age' field from new response
...
//       // 🛠️ UPDATED INDUSTRY LOGIC: Match against the new industries_worked array
//       const matchesIndustry =
//         (filters.industries?.length || 0) === 0 ||
//         filters.industries.some((filterName) => {
//           // Check if any industry in the candidate's worked list matches the filter
//           return (c.industries_worked || []).some(
//             (ind) => ind.name?.toUpperCase() === filterName.toUpperCase(),
//           );
//         });

//       const matchesDepartment =
//         filters.departments.length === 0 ||
//         filters.departments.some((filterName) => {
//           // Direct comparison with the mapped departmentName field
//           return (
//             (c.departmentName || "").toUpperCase() === filterName.toUpperCase()
//           );
//         });

//       // 🛠️ UPDATED LANGUAGE LOGIC: Point to languages_spoken from response
//       const rawCandidateLangs = c.languages_spoken || [];
//       const candidateLangs = rawCandidateLangs.map((l) =>
//         l?.toString().trim().toUpperCase(),
//       );
...
//   }, [candidates, searchQuery, filters, industries, departments]);
  ...
  // This replaces the massive useMemo block you had
...
if (activeFilters.experiences.length > 0) {
  ...
  {isFilterSidebarOpen && (
    <div className="fixed inset-0 z-[250] mb-0 flex justify-end">
      {/* 🌑 Backdrop */}
      <div 
      ...
      {/* 📂 Sidebar Container */}
      <div className="relative w-full max-w-sm bg-white shadow-[-20px_0_50px_rgba(0,0,0,0.1)] h-full flex flex-col animate-in slide-in-from-right duration-500">
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Filter</h3>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">Candidate Filter Data</p>
            </div>
          </div>
          <button 
            onClick={() => setIsFilterSidebarOpen(false)}
            className="p-2 hover:bg-white rounded-xl text-slate-400 hover:text-slate-900 border border-transparent hover:border-slate-200 transition-all"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
          ...
          {/* Active Badges (Your existing badge strip logic) */}
          {Object.values(filters).flat().length > 0 && (
            <div className="space-y-3">
               <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Applied Nodes</span>
                  <button onClick={clearAllFilters} className="text-[9px] font-black text-red-500 hover:underline uppercase">Clear All</button>
               </div>
               <div className="flex flex-wrap gap-2">
                 {Object.entries(filters).map(([category, values]) =>
                   values.map((val) => (
                     <span key={val} className="flex items-center gap-1.5 px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-[9px] font-bold uppercase border border-blue-100">
                       {val} <X size={10} className="cursor-pointer" onClick={() => removeFilter(category, val)} />
                     </span>
                   ))
                 )}
               </div>
          ...
          <div className="space-y-6">
            ...
            <FilterDropdown label="Industry" options={industries.map((i) => i.name.toUpperCase())} onChange={(v) => toggleFilter("industries", v)} selected={filters.industries} />
            <FilterDropdown label="Department" options={departments.map((d) => d.name.toUpperCase()).sort()} onChange={(v) => toggleFilter("departments", v)} selected={filters.departments} />
          </div>
        </div>
        ...
        <div className="p-8 bg-slate-50 border-t border-slate-100">
          <button 
            onClick={() => setIsFilterSidebarOpen(false)}
            className="w-full py-4 bg-slate-900 text-white rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] hover:bg-black transition-all shadow-xl shadow-slate-900/20 active:scale-95 flex items-center justify-center gap-2"
          >
            Apply Filters
          </button>
        </div>
  ...
  {loadingCandidates ? (
    ...
    <div className="flex flex-col items-center justify-center py-32 bg-white border border-slate-100 rounded-xl shadow-sm animate-pulse">
      ...
      <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest mt-2">
        Fetching all candidate information
      </p>
    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 ml-14">
                      ...
                      <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <button
                          ...
                          className="flex-1 sm:flex-none flex items-center justify-center gap-3 border-2 !border-blue-500 px-8 py-2.5 !bg-white !text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] hover:!bg-white transition-all shadow-sm shadow-slate-200 active:scale-95"
                        >
...
const SourceCard = ({ icon, title, desc, color, isAction }) => {
  ...
  return (
    <div
      className={`bg-white p-5 rounded-xl border border-slate-200 shadow-sm transition-all group ${isAction ? "cursor-pointer hover:border-blue-400 hover:shadow-blue-100 hover:-translate-y-1" : ""}`}
    >
      <div className="flex items-center gap-4">
        <div
...
export default CandidateIntakeDummy;
