...
const CreateAttendaceTemplate = () => {
  ...
  const [settings, setSettings] = useState({
    holidayAttendance: 'Do NOT Allow', // Default state
    trackInOut: true,
    noPunchOut: false,
  ...
  // 🚀 UPDATED: Map `editData` properties into the state
  useEffect(() => {
    if (editData) {
      setTemplateName(editData.name || '');

      // 1. Map Attendance Mode
      const modeMap = {
        'Manual': 'manual',
        'Auto Present': 'default',
        'Location': 'location',
        'Selfie + Location': 'selfie'
      };
      if (editData.attendance_mode) {
        setSelectedMode(modeMap[editData.attendance_mode] || 'manual');
      }

      // Map strict backend strings back to UI representations
      const holidayMap = {
        'Do Not Allow': 'Do NOT Allow',
        'Comp Off': 'Comp Off',
        'Allow Attendance': 'Allow attendance'
      };

      const workingHrsReverseMap = {
        'Do not show': 'do_not_show',
        'First In Last Out': 'show_effective',
        'Sum of all punches': 'show_total'
      };

      // 2. Map Toggles & Settings
      setSettings(prev => ({
        ...prev,
        trackInOut: editData.track_in_out_time ?? prev.trackInOut,
        noPunchOut: editData.require_punch_out ?? prev.noPunchOut,
        multiplePunches: editData.allow_multiple_punches ?? prev.multiplePunches,
        autoApprove: editData.auto_approve_enabled ?? prev.autoApprove,
        markAbsent: editData.mark_absent_past_days ?? prev.markAbsent,
        holidayAttendance: holidayMap[editData.attendance_on_holidays] || 'Do NOT Allow',
        effectiveWorkingHours: workingHrsReverseMap[editData.working_hours_rule] || 'do_not_show'
      }));

      // 3. Map Auto Approve Items & Days
      setAutoApproveData(prev => ({
        ...prev,
        approvalDays: editData.auto_approve_days || '',
        attendanceItems: (editData.auto_approve_items || []).filter(item => 
          ['Lens Attendance', 'Location Attendance', 'Selfie & Location Attendance'].includes(item)
        ),
        automationItems: (editData.auto_approve_items || []).filter(item => 
          ['Overtimes', 'Fines'].includes(item)
        )
      }));
    }
  }, [editData]);
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Convert UI keys back to Backend Strict Strings
      const modePayloadMap = {
        'default': 'Auto Present',
        'manual': 'Manual',
        'location': 'Location',
      ...
      const workingHoursMap = {
        ...
        'show_effective': 'First In Last Out',
        'show_total': 'Sum of all punches'
      };

      let holidayRule = "Do Not Allow";
      if (settings.holidayAttendance === "Comp Off") holidayRule = "Comp Off";
      if (settings.holidayAttendance === "Allow attendance") holidayRule = "Allow Attendance";

      // Build JSON Payload exactly as requested by backend
      const payload = {
        name: templateName,
        description: "Attendance configuration template",
        is_active: true,
        attendance_mode: modePayloadMap[selectedMode] || "Manual",
        track_in_out_time: settings.trackInOut,
        require_punch_out: settings.noPunchOut,
        allow_multiple_punches: settings.multiplePunches,
        attendance_on_holidays: holidayRule,
        attendance_on_weekly_offs: holidayRule, // Defaulting to same as holidays per standard logic
        auto_approve_enabled: settings.autoApprove,
        auto_approve_days: parseInt(autoApproveData.approvalDays) || 0,
        auto_approve_items: [...autoApproveData.attendanceItems, ...autoApproveData.automationItems],
        mark_absent_past_days: settings.markAbsent,
        working_hours_rule: workingHoursMap[settings.effectiveWorkingHours] || "Do not show",
        geofence_enabled: false,
        geofence_radius_meters: 0,
        ip_restriction_enabled: false,
        allowed_ips: [],
        late_mark_grace_minutes: 0,
        half_day_trigger_minutes: 0,
        is_overtime_applicable: false
      };

      // Determine Endpoint based on Create or Edit (PATCH)
      const url = editData 
        ? `https://uathr.goelectronix.co.in/attendance-templates/${editData.id}` 
        : `https://uathr.goelectronix.co.in/attendance-templates/`;
      ...
      if (response.ok) {
        toast.success(`Template ${editData ? 'updated' : 'created'} successfully!`, { position: 'top-center' });
        setTimeout(() => navigate('/attendancetemplate'), 500);
      } else {
        const errorData = await response.json();
        console.error("API Error:", errorData);
        toast.error("Failed to save template. Please check required fields.", { position: 'top-center' });
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-white font-['Inter'] pb-32">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-slate-100 px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(-1)} 
             className="p-1.5 hover:!bg-slate-50 rounded-lg !text-black transition-all !bg-transparent border-0 cursor-pointer">
            <ArrowLeft size={18} />
          </button>
          <div>
      ...
      <div className="mx-auto px-1 md:px-4 mt-4 space-y-3">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm">
          ...
          <input 
                ...
                className="w-full bg-slate-50 border border-slate-400 rounded-lg px-3 py-2 text-xs font-semibold text-black outline-none focus:border-blue-600 transition-all placeholder:text-black"
              />
              </div>
            </div>
            ...
            {['Do NOT Allow', 'Comp Off', 'Allow attendance'].map((opt) => (
              <label key={opt} className="flex items-center w-full  gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer border border-transparent transition-colors">
                <input 
                  ...
                  // ✅ Fixed: Properly bound to state
                  checked={settings.holidayAttendance === opt}
                  onChange={() => setSettings(prev => ({ ...prev, holidayAttendance: opt }))}
                />
...
<div className="space-y-1.5 relative"> {/* 🔥 Parent MUST be relative */}
  ...
  >
    ...
    <ChevronDown size={14} className={`text-black transition-transform ${activeDropdown === 'attendance' ? 'rotate-180' : ''}`} />
  </div>
    ...
    <>
      ...
      <div className="absolute top-full mr-4 left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-2 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="space-y-1.5 relative">
          ...
          >
            ...
            <ChevronDown size={14} className={`text-black transition-transform ${activeDropdown === 'automation' ? 'rotate-180' : ''}`} />
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 p-3 flex justify-end shadow-[0_-10px_40px_rgba(0,0,0,0.04)] z-40">
        <div className=" w-full mx-auto flex justify-end gap-3">
          ...
          onClick={handleSave}
          disabled={isSaving}
           className={`flex items-center gap-2 border-2 border-blue-600 px-12 py-2.5 !bg-white !text-blue-600 rounded-lg text-[11px] font-black capitalize tracking-widest shadow-sm shadow-blue-600/30 active:scale-95 transition-all cursor-pointer ${isSaving ? 'opacity-50 pointer-events-none' : ''}`}>
            <Save size={14} /> {isSaving ? 'Saving...' : (editData ? 'Update Template' : 'Save Template')}
          </button>
        </div>
      </div>
    </div>
...
const CheckboxOption = ({ label, checked, onChange, isHeader }) => (
  <label className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${isHeader ? 'bg-slate-50 mb-1 border-b border-slate-100' : 'hover:bg-slate-50'}`}>
    <input 
      ...
      checked={checked}
      onChange={onChange}
      className="w-3.5 h-3.5 rounded border-slate-300 mr-2 text-blue-600 accent-blue-600 focus:ring-0 cursor-pointer" 
    />
...
//   // New states for the additional toggles
//   const [settings, setSettings] = useState({
//     holidayAttendance: 'no_allow', // default string
//     trackInOut: true,
//     noPunchOut: false,
//     multiplePunches: false,
//     autoApprove: false,
...
//       // 1. Map Attendance Mode
//       // (Assuming backend sends things like "Manual", "Selfie + Location", "Location Based", etc.)
//       const modeMap = {
//         'Manual': 'manual',
//         'Mark Present by Default': 'default',
...
//         autoApprove: editData.auto_approve_enabled ?? prev.autoApprove,
//         markAbsent: editData.mark_absent_past_days ?? prev.markAbsent,
//         // (Assuming you'll map working_hours_rule to effectiveWorkingHours later)
//       }));
...
//         ...prev,
//         approvalDays: editData.auto_approve_days || '',
//         // The API returns a mixed array of items like ["Regularization", "Outdoor Duty", "Overtimes"]
//         // You'll need to sort them into your two local state arrays if you keep this exact UI design.
//         // For now, I am passing them into the UI as-is if they match your predefined options.
//         attendanceItems: (editData.auto_approve_items || []).filter(item => 
//           ['Lens Attendance', 'Location Attendance', 'Selfie & Location Attendance'].includes(item)
//         ),
//         automationItems: (editData.auto_approve_items || []).filter(item => 
...
//       // Note: `holidayAttendance` mapping depends on how you want to read `attendance_on_holidays` ("Do Not Allow", "Comp Off") 
//       // back into your radio buttons.
//     }
//   }, [editData]);
...
//                   name="holiday" 
//                   className="w-3 h-3 text-blue-600 accent-blue-600" 
//                   // Temporary hookup for visual consistency with editData
//                   checked={
//                     (opt === 'Do NOT Allow' && editData?.attendance_on_holidays === 'Do Not Allow') ||
//                     (opt === 'Comp Off' && editData?.attendance_on_holidays === 'Comp Off') ||
//                     (opt === 'Allow attendance' && editData?.attendance_on_holidays === 'Allow attendance')
//                   }
//                   onChange={() => {}}
//                 />
//                 <span className="text-[10px] ml-1 font-bold text-black capitalize tracking-wide">{opt}</span>
...
//            className="flex items-center gap-2 border-2 border-blue-600 px-12 py-2.5 !bg-white !text-blue-600 rounded-lg text-[11px] font-black capitalize tracking-widest shadow-sm shadow-blue-600/30 active:scale-95 transition-all cursor-pointer">
//             <Save size={14} /> {editData ? 'Update Template' : 'Save Template'}
//           </button>
//         </div>
//       </div>
//     </div>
...
//           <button
//             onClick={() => navigate(-1)} 
//              className="p-1.5 hover:!bg-slate-50 rounded-lg !text-black transition-all !bg-transparent border-0">
//             <ArrowLeft size={18} />
//           </button>
//           <div>
//             <h1 className="md:text-sm text-[10px] font-black !text-black !capitalize tracking-tight">
