      ...
      <div className="bg-white border-b border-slate-100 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h2 className="text-sm font-black !text-black uppercase tracking-tight">Template</h2>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-4">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
             ...
             <div className='gap-2'>
               <h1 className="md:text-2xl text-lg font-black !text-black tracking-tighter uppercase">Attendance Templates</h1>
              <p className="md:text-[10px] text-[8px] font-bold text-black uppercase tracking-[0.15em] max-w-md">
        ...
        {/* <div className="grid grid-cols-1 gap-3">
          {templates.map((item) => (
            ...
            >
              ...
              <div className="absolute -bottom-6 -right-6 text-black-100 opacity-[0.4] group-hover:text-blue-50 transition-colors -rotate-12">
              ...
              <div className="flex items-start justify-between relative z-10">
                <div className="space-y-4 w-full">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-black text-black-800 uppercase tracking-tight group-hover:text-blue-600 transition-colors">
                      {item.name}
                    </h3>
                    {item.isNew && (
          ...
          <div className="relative">
                  <button 
                    ...
                    className={`p-2 rounded-xl transition-all border-0 outline-none bg-transparent ${
                      activeMenu === item.id ? "text-blue-600 bg-blue-50" : "text-black-300 hover:text-black-900 hover:bg-slate-50"
                    }`}
                  >
     ...
     {/* 🧩 Watermark (Clipped manually via rounded corners if needed, but now allows menu overlap) */}
      ...
      <div className="flex items-start justify-between relative z-10">
         <div className="space-y-4">
          <div className="flex items-center gap-3">
            <div className="w-1.5 h-6 bg-blue-600 rounded-full group-hover:h-8 transition-all" />
            <h3 className="text-sm font-black !text-black uppercase tracking-tight group-hover:text-blue-600 transition-colors">{item.name}</h3>
          </div>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-xl bg-white shadow-2xl z-80 transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full relative">
          ...
          <div className="p-6 border-b border-slate-100 flex justify-between items-start">
            <div>
              <h2 className="text-xl font-black text-black uppercase tracking-tighter">Staff List</h2>
              <p className="text-[11px] font-bold text-black uppercase tracking-widest mt-1">Check staff assigned and unassigned to this template</p>
            </div>
            <button onClick={onClose} className="p-2 hover:!bg-slate-50 rounded-xl !text-black !bg-transparent transition-all"><X size={20} /></button>
          </div>
              ...
              {['selected', 'unselected'].map((tab) => (
                <button
                  ...
                  className={`relative px-6 py-2 rounded-lg text-[10px] font-black !bg-transparent uppercase tracking-widest transition-all duration-300 ${
                    subTab === tab ? '!bg-white shadow-md !text-blue-600 ring-1 ring-slate-200/50' : '!text-black hover:!text-black hover:bg-slate-200/50'
                  }`}
                >
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-slate-50">
                  ...
                  <th className="text-[9px] font-black text-black uppercase tracking-widest py-3">Name</th>
                  <th className="text-[9px] font-black text-black uppercase tracking-widest py-3">Staff ID</th>
                  <th className="text-[9px] font-black text-black uppercase tracking-widest py-3">Base Location</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {currentStaffList.map((staff, idx) => (
                  <tr key={idx} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center text-black"><User size={16} /></div>
                        <span className="text-[11px] font-bold text-black">{staff.name}</span>
                      </div>
                    </td>
                    <td className="py-3 text-[10px] font-black text-blue-600 tracking-tighter uppercase">{staff.id}</td>
                    <td className="py-3 text-[10px] font-medium text-black max-w-[150px] truncate">{staff.location}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-[440px] rounded-[32px] shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in slide-in-from-bottom-4 duration-300">
            <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-white">
              <div>
                <h3 className="text-xl font-black text-black-900 uppercase tracking-tighter">Filter By</h3>
                <div className="h-1 w-8 bg-blue-600 rounded-full mt-1" />
              </div>
              <button onClick={() => setIsFilterOpen(false)} className="p-2.5 bg-slate-50 hover:bg-red-50 hover:text-red-500 rounded-2xl text-black-400 transition-all"><X size={20} /></button>
            </div>
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto text-left">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-black-400 uppercase tracking-[0.2em] ml-1">Salary Type</label>
                <div className="relative">
                  ...
                  <ChevronUp className="absolute right-5 top-1/2 -translate-y-1/2 rotate-180 text-black-300 pointer-events-none" size={16} />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-black-400 uppercase tracking-[0.2em] ml-1">Available Shifts</label>
                <div className="border border-slate-100 rounded-[24px] overflow-hidden bg-white shadow-sm ring-1 ring-slate-200/50">
                  <div className="px-5 py-4 bg-slate-50 flex justify-between items-center border-b border-slate-100">
                    <span className="text-[11px] font-black text-black-500 uppercase tracking-wider">Shift Options</span>
                  </div>
                  <div className="p-3 space-y-1 bg-white">
                    {['Select All', 'IT Staff', '9:30 to 6:30', '11 to 8', '10 to 7'].map((shift, i) => (
                      <label key={i} className="flex items-center gap-4 p-3 hover:bg-blue-50/50 rounded-xl cursor-pointer transition-all group border border-transparent hover:border-blue-100">
                        ...
                        <span className="text-[12px] font-bold text-black-600 group-hover:text-blue-700">{shift}</span>
                      </label>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-black-400 uppercase tracking-[0.2em] ml-1">Department</label>
                <div className="relative">
                  <select className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-[12px] font-bold text-black-700 appearance-none outline-none">
            ...
            <div className="p-6 bg-slate-50/80 border-t border-slate-100 flex gap-4">
              <button onClick={() => setIsFilterOpen(false)} className="flex-1 py-4 border-2 border-slate-200 rounded-2xl text-[11px] font-black uppercase tracking-widest text-black-400 hover:bg-white hover:text-black-600 transition-all">Reset</button>
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-2">
    ...
    <div className="relative bg-white w-full max-w-[360px] rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-slate-100 overflow-hidden animate-in fade-in zoom-in duration-300">
      ...
      <div className="px-4 py-3 border-b border-slate-50 flex justify-between items-center bg-white">
        <div>
          <h3 className="text-[13px] font-black text-black uppercase tracking-tighter">Filter By</h3>
      ...
      <div className="p-4 space-y-3 max-h-[70vh] overflow-y-auto text-left custom-scrollbar">
        ...
        {[
          ...
          <div key={section.id} className="space-y-1">
            <label className="text-[9px] font-black text-black uppercase tracking-[0.15em] ml-0.5">{section.label}</label>
            <div className={`border rounded-xl overflow-hidden transition-all duration-300 bg-white shadow-sm ${openSections === section.id ? 'ring-2 ring-blue-500/10 border-blue-200' : 'border-slate-100 ring-1 ring-slate-200/50'}`}>
               ...
               >
                  <span className={`text-[10px] font-bold ${openSections === section.id ? 'text-blue-600' : 'text-black'}`}>
      ...
      <div className="p-3 flex justify-end bg-slate-50 border-t border-slate-100 gap-2">
        {/* <button 
          onClick={() => setIsFilterOpen(false)}
          className="flex-1 py-2.5 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-black-400 hover:bg-white hover:text-black-600 transition-all active:scale-95"
        >
          Clear Filter
        </button> */}
        <button 
...
const MetaInfo = ({ icon, label, value, isLink }) => (
  <div className="flex items-center gap-2.5">
    <div className={`p-1.5 rounded-lg ${isLink ? 'bg-blue-100 text-blue-600' : 'bg-white text-black shadow-sm border border-slate-100'}`}>
      {icon}
    </div>
    <div>
      <p className="text-[9px] font-black text-black-400 uppercase tracking-[0.15em] leading-none mb-1">
        {label}
      </p>
      <p className={`text-[10px] font-bold uppercase leading-none ${isLink ? 'text-blue-600 underline cursor-pointer' : 'text-black'}`}>
        {value}
      </p>
    </div>
  </div>
...
const MenuOption = ({ icon, label, onClick, variant = 'default' }) => (
  ...
  >
    ...
    <span className={`text-[9px] font-black uppercase tracking-[0.15em] ${
      variant === 'danger' ? 'text-red-500/70 group-hover:text-red-600' : 'text-black group-hover:text-black'
    }`}>
      {label}
    </span>
  </button>
...
//             <h2 className="text-sm font-black text-black-900 uppercase tracking-tight">Template</h2>
//           </div>
//         </div>
//       </div>
...
//               </div>
//              <div className='gap-2'>
//                <h1 className="text-2xl font-black text-black-900 tracking-tighter uppercase">Attendance Templates</h1>
//               <p className="text-[10px] font-bold text-black-400 uppercase tracking-[0.15em] max-w-md">
//                attendance modes, late entry, attendance on holiday and More.
//             </p>
//              </div>
//             </div>
...
//                 <button className="p-2 !text-black-300 hover:!text-black-900 hover:!bg-slate-50 !bg-transparent rounded-xl transition-all border-0 outline-none">
//                   <MoreVertical size={20} />
//                 </button>
//               </div>
//             </div>
...
// const MetaInfo = ({ icon, label, value, isLink }) => (
//   <div className="flex items-center gap-2.5">
//     <div className={`p-1.5 rounded-lg ${isLink ? 'bg-blue-100 text-blue-600' : 'bg-white text-black-400 shadow-sm border border-slate-100'}`}>
//       {icon}
//     </div>
//     <div>
//       <p className="text-[9px] font-black text-black-400 uppercase tracking-[0.15em] leading-none mb-1">
//         {label}
//       </p>
//       <p className={`text-[10px] font-bold uppercase leading-none ${isLink ? 'text-blue-600 underline cursor-pointer' : 'text-black-600'}`}>
//         {value}
//       </p>
//     </div>
//   </div>
